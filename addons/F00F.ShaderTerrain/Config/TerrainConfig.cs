...
public partial class TerrainConfig : CustomResource
{
    ...
    public MeshInstance3D CreateChunk(in ChunkId id, int lod) => new()
    {
        Name = $"Chunk {id}|{lod}",
        Position = (id * ChunkSize).FromXZ(),
        ExtraCullMargin = Amplitude,
        Mesh = New.PlaneMesh(ChunkSize, lod, ShaderMaterial),
    };

    public StaticBody3D CreateShape(in ChunkId id)
    {
        ...
        StaticBody3D NewBody(in ChunkId id) => new()
        {
            Name = $"Body {id}",
            Position = pos.FromXZ(),
        };
